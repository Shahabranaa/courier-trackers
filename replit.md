...
## Recent Changes
- **2026-02-11**: Fixed avg delivery days by city showing empty: used transactionDate as fallback when lastStatusTime is null for delivered orders. Previous lastFetchedAt fallback gave inflated 22+ day averages because it measured sync time, not delivery time. PostEx transactionDate reflects status change date. Tranzo orders without lastStatusTime excluded (no data better than wrong data).
