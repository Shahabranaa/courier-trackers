...
- **WhatsApp Integration (WeTarSeel):** Dashboard page (`/whatsapp`) fetches conversations and messages from WeTarSeel API (`bun-prod-new.app.wetarseel.ai`) — no standalone server needed. Brand settings store `wetarseelAccountId` and `wetarseelUserId` (from WeTarSeel URL params). APIs: `/api/whatsapp/messages` (list conversations), `/api/whatsapp/chat` (get messages for a conversation), `/api/whatsapp/status` (check config), `/api/whatsapp/convert` (create Shopify order from WhatsApp convo). UI is a WhatsApp-style read-only inbox: conversation list on the left, message detail on the right, with "Create Order" button per conversation. Reply feature removed (WeTarSeel free plan doesn't support sending). **Smart Detection:** Conversations are analyzed for Pakistani city names (150+ cities) and phone numbers (03xx patterns). Unreplied conversations with city/phone signals are marked as "high priority" (red border), unreplied without signals as "medium" (amber border), and conversations with signals but already replied as "low" (blue border). Badges show detected city name, phone presence, and "No Reply" status. Filter tabs: All / Unreplied / Needs Attention. "Needs Attention" counter shown in header. When a chat is opened, messages are deep-scanned for signals from earlier in the conversation (not just last message). Detected city auto-fills the shipping city when creating an order. Individual messages with city/phone mentions get highlighted borders and signal badges below them.