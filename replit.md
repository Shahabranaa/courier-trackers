...
- **WhatsApp Integration (WeTarSeel):** Dashboard page (`/whatsapp`) fetches conversations and messages from WeTarSeel API (`bun-prod-new.app.wetarseel.ai`) — no standalone server needed. Brand settings store `wetarseelAccountId`, `wetarseelUserId` (from WeTarSeel URL params), and `wetarseelAuthToken` (pb_auth JWT for sending replies). APIs: `/api/whatsapp/messages` (list conversations), `/api/whatsapp/chat` (get messages for a conversation), `/api/whatsapp/status` (check config), `/api/whatsapp/convert` (create Shopify order from WhatsApp convo), `/api/whatsapp/reply` (send reply via WeTarSeel server action, requires auth). UI is a WhatsApp-style inbox: conversation list on the left, message detail on the right, with reply input bar and "Create Order" button per conversation. Reply uses WeTarSeel's Next.js server action at `app.wetarseel.ai/{accountId}/live-chat` with the `next-action` header and `pb_auth` cookie.