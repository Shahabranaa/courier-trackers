...
## Recent Changes
- **2026-02-09**: Added authentication system with username/password login. Admin creates first account via one-time setup page. Admin panel at /admin for managing users (create/edit/delete, assign roles, assign brands). Users only see brands assigned to them by admin. Login page at /login, setup page at /setup. Session-based auth with secure cookies. Settings page restricted to admin only. Logout button in sidebar. User/UserBrand models in Prisma. API routes: /api/auth/login, /api/auth/logout, /api/auth/me, /api/auth/setup, /api/admin/users, /api/brands/mine. BrandContext now auth-aware.
