...
## Recent Changes
- **2026-02-12**: Fixed avg delivery days by city showing empty: fallback chain is lastStatusTime → transactionDate → lastFetchedAt. PostEx/Tranzo sync now captures lastStatusTime when orders transition to Delivered/Returned status. Future syncs will produce accurate delivery times. Widget subtitle notes "Sync data for accurate times".
