...
## Recent Changes
- **2026-02-09**: Migrated Shopify auth from static access token to OAuth Client Credentials Grant flow. Brand model now stores shopifyClientId + shopifyClientSecret (replacing shopifyAccessToken). Server exchanges credentials for short-lived tokens (cached 23h). Client Secret masked in API responses.
