...
export default function SyncToast({ summary, onClose, courier }: SyncToastProps) {
  ...
  const accentColor = courier === "PostEx" ? "orange" : courier === "TCS" ? "red" : "purple";
  ...
  return (
    ...
    >
      <div className={`bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden`}>
        <div className={`px-4 py-3 flex items-center justify-between ${accentColor === "orange" ? "bg-orange-50 border-b border-orange-100" : accentColor === "red" ? "bg-red-50 border-b border-red-100" : "bg-purple-50 border-b border-purple-100"}`}>
          <div className="flex items-center gap-2">
            <CheckCircle2 size={18} className={accentColor === "orange" ? "text-orange-600" : accentColor === "red" ? "text-red-600" : "text-purple-600"} />
            <span className={`font-semibold text-sm ${accentColor === "orange" ? "text-orange-800" : accentColor === "red" ? "text-red-800" : "text-purple-800"}`}>
        ...
        <div className={`h-1 ${accentColor === "orange" ? "bg-orange-500" : accentColor === "red" ? "bg-red-500" : "bg-purple-500"} transition-all duration-[8000ms] ease-linear ${visible ? "w-0" : "w-full"}`}></div>
      </div>
    </div>
  );
}
