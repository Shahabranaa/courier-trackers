...
import { LayoutDashboard, Truck, Package, Settings, LogOut, ChevronLeft, ChevronRight, PieChart, ChevronDown, Plus, Building2, ShoppingBag, TrendingUp, Bell, Zap, GitCompare, Receipt, Wallet, Users, Shield, UserPlus, MessageCircle, CreditCard } from "lucide-react";
            ...
            <div className="p-3 border-t border-gray-100 space-y-1">
                {authUser?.role === "ADMIN" && (
                    <>
                        ...
                        <Link href="/admin/employees" className={`flex items-center gap-3 w-full p-2 hover:bg-amber-50 rounded-xl transition-colors group ${pathname === "/admin/employees" ? "bg-amber-50" : ""}`}>
                            <div className={`h-9 w-9 rounded-full flex items-center justify-center border transition-colors ${pathname === "/admin/employees" ? "bg-amber-100 border-amber-200 text-amber-600" : "bg-gray-100 border-gray-200 text-gray-500 group-hover:border-amber-200 group-hover:text-amber-600"}`}>
                                <UserPlus size={18} />
                            </div>
                            {!collapsed && (
                                <div className="text-left overflow-hidden">
                                    <p className="text-sm font-medium text-gray-900 group-hover:text-amber-700">Employees</p>
                                    <p className="text-xs text-gray-500 truncate">Order creators</p>
                                </div>
                            )}
                        </Link>
                        <Link href="/admin/brands" className={`flex items-center gap-3 w-full p-2 hover:bg-amber-50 rounded-xl transition-colors group ${pathname === "/admin/brands" ? "bg-amber-50" : ""}`}>
                            <div className={`h-9 w-9 rounded-full flex items-center justify-center border transition-colors ${pathname === "/admin/brands" ? "bg-amber-100 border-amber-200 text-amber-600" : "bg-gray-100 border-gray-200 text-gray-500 group-hover:border-amber-200 group-hover:text-amber-600"}`}>
                                <CreditCard size={18} />
                            </div>
                            {!collapsed && (
                                <div className="text-left overflow-hidden">
                                    <p className="text-sm font-medium text-gray-900 group-hover:text-amber-700">Subscriptions</p>
                                    <p className="text-xs text-gray-500 truncate">Activate brands</p>
                                </div>
                            )}
                        </Link>
                    </>
