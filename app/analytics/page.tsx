...
import {
  ...
  Users, UserCheck, UserX, Crown, AlertTriangle, Phone, Search, Download
} from "lucide-react";
...
interface PerformanceData {
  ...
  cityDeliveryRates: { city: string; total: number; delivered: number; returned: number; deliveryRate: number; returnRate: number; avgFeeTax: number | null; couriers: Record<string, { total: number; delivered: number; deliveryRate: number; avgFeeTax: number | null }> }[];
}
...
function formatCurrencyPrecise(value: number | null): string {
  return value == null ? "—" : `Rs. ${value.toLocaleString("en-PK", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}
...
export default function AnalyticsPage() {
  ...
  const exportCityBreakdown = () => {
    if (!data || !data.cityBreakdown.length) return;
    exportToCsv(
      `City_Breakdown_${selectedMonth}.csv`,
      ["#", "City", "Orders", "Share (%)", "Revenue (PKR)"],
      data.cityBreakdown.map((c, i) => [i + 1, c.city, c.count, c.percentage, c.revenue])
    );
  };

  const exportCityDeliveryRates = () => {
    if (!perfData || !perfData.cityDeliveryRates.length) return;
    const allCouriers = Array.from(new Set(perfData.cityDeliveryRates.flatMap(c => Object.keys(c.couriers)))).sort();
    const headers = ["#", "City", "Total", "Delivered", "Returned", "Delivery Rate (%)", "Return Rate (%)", "Avg Delivery Fee + Tax (PKR)", ...allCouriers.flatMap(c => [`${c} Total`, `${c} Delivered`, `${c} Rate (%)`, `${c} Avg Fee + Tax (PKR)`])];
    exportToCsv(
      `City_Delivery_Rates_${selectedMonth}.csv`,
      headers,
      perfData.cityDeliveryRates.map((c, i) => [
         i + 1, c.city, c.total, c.delivered, c.returned, c.deliveryRate, c.returnRate, c.avgFeeTax ?? "",
         ...allCouriers.flatMap(co => [c.couriers[co]?.total ?? "", c.couriers[co]?.delivered ?? "", c.couriers[co]?.deliveryRate ?? "", c.couriers[co]?.avgFeeTax ?? ""])
      ])
    );
  };

  const exportCustomers = (tab: "repeat" | "ltv" | "problem") => {
    if (!customerData) return;
    if (tab === "repeat") {
      exportToCsv(
        `Repeat_Customers_${selectedMonth}.csv`,
        ["#", "Phone", "Name", "City", "Orders", "Total Revenue (PKR)", "First Order", "Last Order"],
        customerData.repeatCustomers.map((c, i) => [i + 1, c.phone, c.name, c.city, c.totalOrders, c.totalRevenue, c.firstOrder, c.lastOrder])
      );
    } else if (tab === "ltv") {
      exportToCsv(
        `Top_LTV_Customers_${selectedMonth}.csv`,
        ["#", "Phone", "Name", "City", "Orders", "Total Revenue (PKR)", "First Order", "Last Order"],
        customerData.topByLTV.map((c, i) => [i + 1, c.phone, c.name, c.city, c.totalOrders, c.totalRevenue, c.firstOrder, c.lastOrder])
      );
    } else {
      exportToCsv(
        `Problem_Customers_${selectedMonth}.csv`,
        ["#", "Phone", "Name", "City", "Orders", "Total Revenue (PKR)", "First Order", "Last Order"],
        customerData.problemCustomers.map((c, i) => [i + 1, c.phone, c.name, c.city, c.totalOrders, c.totalRevenue, c.firstOrder, c.lastOrder])
      );
    }
  };
                    ...
                    <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                      ...
                      <Tooltip
                        ...
                        formatter={(value, name) => {
                          ...
                          return [Number(value ?? 0), labels[nameStr] || nameStr];
                        }}
                      />
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="lg:w-80 shrink-0">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        onClick={exportCityBreakdown}
                        className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                      >
                              ...
                              <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={perfData.courierComparison} barCategoryGap="30%">
                                  ...
                                  <Tooltip
                                    ...
                                    formatter={(value, name) => [`${Number(value ?? 0)}%`, name === "deliveryRate" ? "Delivery Rate" : "Return Rate"]}
                                  />
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={perfData.courierComparison} barCategoryGap="25%">
                              ...
                              <Tooltip
                                ...
                                formatter={(value, name) => {
                                  const nameStr = String(name ?? "");
                                  const label = nameStr === "deliveryRate" ? "Delivery Rate" : nameStr === "returnRate" ? "Return Rate" : nameStr;
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm mb-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={exportCityDeliveryRates}
                          className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                        >
                              ...
                              <thead>
                                <tr className="text-xs text-gray-500 uppercase border-b border-gray-100">
                                  ...
                                  <th className="text-left py-2.5 px-3 font-semibold">City</th>
                                   ...
                                   <th className="text-right py-2.5 px-3 font-semibold">All Couriers Avg Fee + Tax</th>
                                  {courierList.map(courier => (
                                  ...
                                  <tr key={city.city} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="py-2.5 px-3 text-center text-gray-600 font-medium">{city.total}</td>
                                     <td className="py-2.5 px-3 text-right">
                                       <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-semibold text-slate-700 bg-slate-50 whitespace-nowrap">
                                         {formatCurrencyPrecise(city.avgFeeTax)}
                                       </span>
                                     </td>
                                    {courierList.map(courier => {
                                          ...
                                          {cd && cd.total > 0 ? (
                                            <div>
                                              ...
                                              <div className="text-[10px] text-gray-400 mt-0.5">{cd.delivered}/{cd.total}</div>
                                               <div className="text-[10px] font-medium text-slate-600 mt-1 whitespace-nowrap">{formatCurrencyPrecise(cd.avgFeeTax)}</div>
                                            </div>
                                          ) : (
                                            <span className="text-gray-300 text-xs">-</span>
                                          )}
            ...
            {customerData && (
              <section>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button
                    onClick={() => exportCustomers(customerTab)}
