...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const user = await prisma.user.create({
      ...
      select: { id: true, email: true, name: true, role: true, isActive: true, lastLoginAt: true, adminNotes: true, createdAt: true, _count: { select: { brands: true } } }
    });
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const { id, isActive, adminNotes } = await req.json();
