...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const order of orders) {
      ...
      const statuses = [order.transactionStatus, order.orderStatus, order.lastStatus]
        .filter(Boolean)
      ...
      const isDelivered = statuses.some((value) =>
        value.includes("deliver")
        || value.includes("transferred")
        || (courier === "TCS" && value === "ok")
      );
      const isReturned = statuses.some((value) =>
        value.includes("return")
        || (courier === "TCS" && ["rs", "ro"].includes(value))
      );
      const isCancelled = statuses.some((value) => value.includes("cancel"));
