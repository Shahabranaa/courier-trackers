  ...
  try {
    ...
    const returnedOrders = await prisma.order.findMany({
      where: {
        ...courierWhere,
        OR: [
          { transactionStatus: { contains: "return", mode: "insensitive" } },
          { transactionStatus: { in: ["RS", "RO", "rs", "ro"] } },
          { orderStatus: { contains: "return", mode: "insensitive" } },
          { orderStatus: { in: ["RS", "RO", "rs", "ro"] } },
          { lastStatus: { contains: "return", mode: "insensitive" } },
          { lastStatus: { in: ["RS", "RO", "rs", "ro"] } },
        ],
      },
      select: {
        trackingNumber: true,
    ...
    if (returnedOrders.length === 0) {
      return NextResponse.json({
        discrepancies: [],
        summary: { total: 0, postex: 0, tranzo: 0, tcs: 0, leopards: 0, totalAmount: 0 },
      });
    }
    ...
    const tcsCount = discrepancies.filter((d) => d.courier === "TCS").length;
    const leopardsCount = discrepancies.filter((d) => d.courier === "Leopards").length;
