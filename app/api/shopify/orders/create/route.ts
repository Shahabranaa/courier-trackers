...
export async function POST(req: NextRequest) {
    ...
    const { customerName, phone, shippingAddress, shippingCity, lineItems, notes, deliveryFee, employeeUsername, brandId: requestedBrandId } = body;
    ...
    if (employeeUsername) {
        ...
        const allowedBrandIds = new Set<string>([
            employee.brandId,
            ...employee.brandAccess.map((ba) => ba.brandId),
        ]);

        const headerBrandId = req.headers.get("brand-id");
        const chosenBrandId = requestedBrandId || headerBrandId || employee.brandId;

        if (!allowedBrandIds.has(chosenBrandId)) {
            return NextResponse.json(
                { error: "You do not have access to the selected brand" },
                { status: 403 }
            );
        }
