...
export async function GET(req: NextRequest) {
    ...
    if (!user) {
        const hasPrimaryEmployee = await prisma.employee.findFirst({
            where: { brandId, isActive: true },
            select: { id: true },
        });
        const hasJoinedEmployee = hasPrimaryEmployee
            ? null
            : await prisma.employeeBrand.findFirst({
                where: { brandId, employee: { isActive: true } },
                select: { id: true },
            });
        if (!hasPrimaryEmployee && !hasJoinedEmployee) {
