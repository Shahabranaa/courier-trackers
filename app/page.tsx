...
export default function UnifiedDashboard() {
  ...
  const fetchData = async (forceSync = false) => {
    ...
    try {
      ...
      if (!forceSync || selectedBrand.tcsCustomerNumber) {
        const tcsParams = new URLSearchParams({
          brandId: selectedBrand.id,
          startDate,
          endDate,
        });
        if (forceSync) tcsParams.set("force", "true");
        promises.push(
          fetch(`/api/tcs/orders?${tcsParams.toString()}`).then(async r => {
            if (r.ok) {
              const data = await r.json();
              return { type: "tcs", data: data.dist || [] };
            }
            return { type: "tcs", data: [] };
          })
        );
      }
      if (!forceSync || selectedBrand.leopardsEnabled) {
        const leopardsParams = new URLSearchParams({ brandId: selectedBrand.id, startDate, endDate });
        if (forceSync) leopardsParams.set("force", "true");
        promises.push(fetch(`/api/leopards/orders?${leopardsParams}`).then(async r => {
          if (r.ok) { const data = await r.json(); return { type: "leopards", data: data.dist || [] }; }
  ...
  const { totalOrders, totalNet, dailyStats } = useMemo(() => {
    ...
    tcsData.forEach(o => {
      ...
      const isDelivered = status.includes("deliver") || status.includes("transferred") || status === "ok";
      const isReturned = status.includes("return") || status === "rs" || status === "ro";
      if (status.includes("cancel")) return;
      dailyMap[day].tcsOrders += 1;
      dailyMap[day].totalOrders += 1;
      totOrders++;
      if (isDelivered) {
        const net = parseFloat(o.netAmount || "0");
        dailyMap[day].tcsNet += net;
        dailyMap[day].totalNet += net;
        totNet += net;
      } else if (isReturned) {
        const fee = parseFloat(o.transactionFee || "0");
        dailyMap[day].tcsNet -= fee;
        dailyMap[day].totalNet -= fee;
        totNet -= fee;
      }
    });
    leopardsData.forEach(o => {
      const day = getDay(o.orderDate || o.transactionDate);
      if (!dailyMap[day]) dailyMap[day] = initDay(day);
      const status = (o.transactionStatus || o.orderStatus || o.lastStatus || "").toLowerCase();
      if (status.includes("cancel") || status.includes("void")) return;
      const delivered = /deliver|completed|\bok\b/.test(status) && !/not delivered|undelivered/.test(status);
      const returned = /return|\brto\b|\bro\b|\brs\b/.test(status);
      dailyMap[day].leopardsOrders++; dailyMap[day].totalOrders++; totOrders++;
      if (delivered) { const net = parseFloat(o.netAmount || o.orderAmount || o.invoicePayment || "0"); dailyMap[day].leopardsNet += net; dailyMap[day].totalNet += net; totNet += net; }
  ...
  const overallStats = useMemo(() => {
    ...
    tcsData.forEach(o => {
      const status = (o.transactionStatus || o.orderStatus || o.lastStatus || "").toLowerCase();
      if (status.includes("cancel")) return;
      if (status.includes("deliver") || status.includes("transferred") || status === "ok") {
        delivered++;
        deliveredRevenue += parseFloat(o.invoicePayment || o.orderAmount || "0");
      } else if (status.includes("return") || status === "rs" || status === "ro") {
        returned++;
      } else {
        inTransit++;
      }
    ...
    leopardsData.forEach(o => {
      ...
      if ((/deliver|completed|\bok\b/.test(status)) && !/not delivered|undelivered/.test(status)) { delivered++; deliveredRevenue += parseFloat(o.invoicePayment || o.orderAmount || "0"); }
      ...
      <div className="flex flex-col gap-8 p-6 lg:p-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            ...
            <Link href="/tcs" className="block bg-gradient-to-br from-red-50 to-white p-5 rounded-2xl border border-red-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-white text-red-600 rounded-xl shadow-sm"><Truck className="w-5 h-5" /></div>
                  <div><h3 className="font-bold text-gray-900">TCS</h3><p className="text-xs text-gray-500">{tcsData.length} orders</p></div>
                </div>
              ...
              <thead className="bg-gray-50/80">
                <tr>
                  ...
                  <th className="text-center border-l border-gray-100" colSpan={2}>
                   ...
                   <th className="text-center border-l border-gray-100" colSpan={2}>
                     <div className="flex items-center justify-center gap-1.5 py-2"><div className="w-2.5 h-2.5 rounded-full bg-red-600"></div><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">TCS</span></div>
                   </th>
                   <th className="text-center border-l border-gray-100" colSpan={2}>
                ...
                {dailyStats.map((day) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3.5 whitespace-nowrap text-right text-xs font-mono text-gray-600">
                     ...
                     <td className="px-3 py-3.5 whitespace-nowrap text-center text-sm border-l border-gray-50">
                       {day.tcsOrders > 0 ? <span className="bg-red-50 text-red-700 px-2 py-0.5 rounded-md text-xs font-bold">{day.tcsOrders}</span> : <span className="text-gray-300">-</span>}
                     </td>
                     <td className="px-3 py-3.5 whitespace-nowrap text-right text-xs font-mono text-gray-600">
                       {day.tcsNet !== 0 ? formatRs(day.tcsNet) : <span className="text-gray-300">-</span>}
                     </td>
                     <td className="px-3 py-3.5 whitespace-nowrap text-center text-sm border-l border-gray-50">
                ...
                {dailyStats.length > 0 && (
                  <tr className="bg-gray-50 font-bold border-t-2 border-gray-200">
                    ...
                    <td className="px-3 py-4 text-right text-xs font-mono text-purple-700">
                     ...
                     <td className="px-3 py-4 text-center text-sm text-red-700 border-l border-gray-100">
                       {dailyStats.reduce((s, d) => s + d.tcsOrders, 0) || '-'}
                     </td>
