...
import {
    ...
    Phone, MapPin, User, Package, FileText, Search, ChevronDown, X,
    ClipboardPaste, Edit3, AlertCircle, Building2
} from "lucide-react";
...
export default function EmployeeCreateOrderPage() {
    ...
    const employeeSlug = params.employee as string;
    ...
    useEffect(() => {
        async function validateEmployee() {
            try {
                const res = await fetch(`/api/employee/${encodeURIComponent(employeeSlug)}`);
                if (!res.ok) {
                    setEmployeeError("This link is not valid or has been deactivated.");
                    setEmployeeLoading(false);
                    return;
                }
                const data = await res.json();
                const brandsList: BrandOption[] = Array.isArray(data.brands) && data.brands.length > 0
                    ? data.brands
                    : [{ id: data.brandId, name: data.brandName }];
                setEmployeeInfo({ ...data, brands: brandsList });
                setSelectedBrandId(data.brandId);
            } catch {
                setEmployeeError("Failed to load. Please try again.");
            }
            setEmployeeLoading(false);
        }
        validateEmployee();
    ...
    useEffect(() => {
        if (!employeeInfo || !selectedBrandId) return;
        setProducts([]);
        setLineItems([]);
        setProductsError(null);
        setLoadingProducts(true);
        fetch("/api/shopify/products", {
            headers: { "brand-id": selectedBrandId }
        })
            .then(r => r.json())
            .then(data => {
                if (data.products) {
            ...
            .catch(() => { setProductsError("Failed to load products. Please check your connection."); })
            .finally(() => setLoadingProducts(false));
    }, [employeeInfo, selectedBrandId]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const submitBrandId = selectedBrandId || employeeInfo.brandId;
        ...
        try {
            const res = await fetch("/api/shopify/orders/create", {
                ...
                body: JSON.stringify({
                    ...
                    employeeUsername: employeeSlug,
                    brandId: submitBrandId,
                }),
            });
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex items-center justify-center p-6">
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center max-w-md">
                    ...
                    <p className="text-sm text-gray-500">{employeeError || "This employee link is not valid."}</p>
                </div>
            </div>
        );
    }
            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
                <header className="bg-white border-b border-gray-100 px-6 py-4">
                    <div className="max-w-3xl mx-auto flex items-center gap-3">
                        ...
                        <span className="text-sm text-gray-500">{employeeInfo.name}</span>
                    </div>
                </header>
                <div className="p-6">
                    <div className="max-w-2xl mx-auto">
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
                            ...
                            <p className="text-gray-500 mb-6">Order has been pushed to Shopify</p>
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
            <header className="bg-white border-b border-gray-100 px-6 py-4 sticky top-0 z-30">
                <div className="max-w-3xl mx-auto flex items-center justify-between">
                    ...
                    {employeeInfo.brands.length > 1 ? (
                        <div className="flex items-center gap-2">
                            <Building2 size={14} className="text-gray-400" />
                            <select
                                value={selectedBrandId}
                                onChange={(e) => setSelectedBrandId(e.target.value)}
                                className="text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-full px-3 py-1.5 pr-7 focus:outline-none focus:ring-2 focus:ring-indigo-400 cursor-pointer appearance-none bg-no-repeat bg-[length:14px] bg-[position:right_0.5rem_center]"
                                style={{
                            ...
                            >
                                {employeeInfo.brands.map((b) => (
                                    <option key={b.id} value={b.id}>{b.name}</option>
                                ))}
                            </select>
                        </div>
                    ) : (
                        <span className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full font-medium">{employeeInfo.brandName}</span>
                    )}
                </div>
            </header>
                    ...
                    {(mode === "manual" || filledFromPaste) && (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                                ...
                                {productsError && (
                                    <div className="mb-3 flex items-start gap-2 rounded-lg bg-red-50 border border-red-200 px-3 py-2.5 text-xs text-red-700">
                                        <AlertCircle size={14} className="shrink-0 mt-0.5" />
                                        <span>
                                            {productsError}
                                            {(productsError.toLowerCase().includes("not configured") || productsError.toLowerCase().includes("no shopify")) && (
                                                <a href="/settings" target="_blank" rel="noopener noreferrer" className="ml-1 underline font-medium">Go to Brand Settings</a>
                                            )}
                                        </span>
                                    </div>
                                )}
